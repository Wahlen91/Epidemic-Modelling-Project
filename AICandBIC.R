...
ModObj <- lapply(models,load,.GlobalEnv)
...
names <- unlist(ModObj)
names <- sub("model.", names, replace="Negative Binomial ")
...
print(xtable(AicBic.df, label="TabelMods", caption="AIC and BIC values for all models.
                     The abbrevation ''as.'' stands for ''Age and Sex stratified'' and ''ba.'' 
                     stands for ''Before and After the official announcement''",
               align="lccc"),type="latex",include.rownames=FALSE, file="AicBicTable.tex")
