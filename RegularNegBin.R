...
est.Smooth <- cbind(Estimate = model.Smooth$coefficients,
                    confint.default(model.Smooth))
...
est.as.Smooth <- cbind(Estimate = model.as.Smooth$coefficients,
                       confint.default(model.as.Smooth))
...
est.as.ba.Smooth <- cbind(Estimate = model.as.ba.Smooth$coefficients,
                   confint.default(model.as.ba.Smooth))
# Exponentiate
...
# Make data.frame with deviance and Anscombe residuals for the models
resid.lst <- lapply(Models, Resid.FUN, AgeClass = alldata$Age, Sex = alldata$Sex)
